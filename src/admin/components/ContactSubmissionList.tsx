...
export default function ContactSubmissionList({ submissions }: { submissions: ContactSubmission[] }) {
  if (submissions.length === 0) {
    return <p className="text-sm text-white/50">No contact submissions.</p>
  }
  return (
      ...
      {submissions.map(s => (
        <li key={s.id} className="rounded-lg border border-white/10 bg-white/[0.04] p-3 text-sm">
          <div className="mb-1 flex items-center justify-between text-[10px] text-white/50">
            ...
            <span className="text-white/70">{formatLong(s.created_at)}</span>
          </div>
          <div className="whitespace-pre-wrap text-white/80">{s.message}</div>
        </li>
      ))}
    </ul>
  )
