...
- `.icon-optical` — optical icon sizing for `Button`. The caller sets `--icon-size`; light geometric glyphs (lucide `arrow*`/`chevron*`/`corner*`/`move*`/`plus*`/`minus*`/`check*`/`x*`, matched on the `lucide-<name>` class lucide stamps on every svg) render at 1.25x so they don't read undersized next to dense pictograms. CSS beats lucide's `size`/`className` on the icon, so `size={14}` or `h-4 w-4` at a call site inside a Button has no effect.
