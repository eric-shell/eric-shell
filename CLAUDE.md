...
- **Both sims are fixed-step** (`t += 0.003` per frame, not delta-timed), which is *why* pausing them bluntly is safe — they freeze and resume with no jump. **Don't convert them to delta-time without revisiting `visibleRafLoop`**: resuming after a long scroll-away would integrate one enormous step. (Fixed-step also means they run ~2x faster on a 120Hz display; pre-existing, and separate from the gating.)
