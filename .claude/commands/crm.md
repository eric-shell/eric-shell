...
- **Persistence is best-effort**. Both `chat.ts` and `contact.ts` write to Postgres *after* responding to the user, wrapped in try/catch. A DB outage logs an error but never breaks the public surface. Don't move persistence in front of the response.
