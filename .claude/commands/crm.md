...
- **No section-impression or in-page click tracking.** Deliberately scoped out: what someone scrolled past and what they clicked is a meaningfully bigger privacy footprint than "which pages, how long". Two narrow exceptions have since been made, both by widening `visitor_events`' `type` check constraint rather than adding tables, which is still the route to take: `outbound_click` (links that *leave* the site) and `filter_apply`. Neither records hovers, impressions, or in-page navigation.
- **`filter_apply` records a settled tag/sort selection** on the work grid or the notes list — the only two surfaces whose state never reaches the URL, so `page_views` cannot tell a narrowed list from a whole one. [src/hooks/useFilterTelemetry.ts](src/hooks/useFilterTelemetry.ts) is the only thing that should send it, and three rules there keep the volume honest: the default state is never reported, the settled *selection* is sent rather than each toggle (800ms, tags sorted so the same pair in either order is one filter), and a state already reported by this page load is not reported again. Undebounced this would write four rows for one four-tag decision and blow the 10/min limit on `/api/events` — which is shared with `outbound_click`, so it would take the click that followed down with it. A settle still pending at unload is flushed on `pagehide`, because filtering and immediately opening a result is the path most worth seeing and the one a bare debounce eats. Surfaced as **Top filters** in `InsightsPanel`, one bar per `(section, tag)` — the two indexes share a tag vocabulary, so a bar labelled `react` alone would be ambiguous.
